...
- **A frozen wallet must name the await it is parked on.** Every network call in the loop carries a timeout, so a freeze that outlasts all of them cannot be reasoned about from the outside — the row reads `proses swap` whether the swap is slow or the coroutine is wedged. `_watchdog` runs beside the wallets, and a wallet whose loop has not advanced in `stall_timeout_seconds` gets `Task.print_stack` written to the log plus one Telegram warning. `_mark_progress` is called once per loop pass; `_park_until_day` marks the deliberate day-long idles so they are not reported as freezes.
- **A guard rejection must not be waited out silently.** Polling is right for a network fee (it moves) and futile for a pool fee (a property of the pool), so `_blocking_guard` reads `GuardResult.details` to name the limit actually biting — the status used to read `wait fee` for every rejection — and `_note_guard_wait` escalates to `stuck: …` plus one Telegram warning past `guard_wait_seconds`. Without it a wallet holding a token it can never sell polls for ever while the others reach target, visible only as a row that never moves.
- **A wallet must never end without saying so.** `run()` reports a returned or crashed wallet only *after* `asyncio.gather` — which waits for every wallet, and a wallet polling a guard may poll until the run is stopped. So a wallet that ends early is reported for the rest of the day by nothing at all, and the dashboard keeps showing its last status. `_run_wallet` therefore catches the crash itself (status `ERROR`, `run_state.finish`, one Telegram line), and the one early `return` left in `_trade_wallet` — the startup CC quote — goes through `_initial_notional`, which retries `PRICE_RETRIES` times and, if it still cannot price, stops the wallet loudly. A single transient quote error used to end a wallet before the loop was entered: no route, no plan, `0/50`, row reading `running` all day. The watchdog cannot cover this — `_run_wallet`'s `finally` drops the wallet from `_last_progress`, so a wallet that *returned* is not a wallet that is stalled.
...
| Trading history, CC rebates | [webclient.py](src/cantex_bot/webclient.py) → `api.cantex.io/v1/history/trading`, `/v1/account/reward_activity` | the SDK's Bearer token, **cached** (`_bearer`) |
...
**`/v1/history/trading` has no paging at all.** Measured 2026-08-21: it returns the 50 newest rows and ignores `limit` and `offset` entirely, with no total or cursor in the response — every combination gave back the same 50, all from the current day. So a poll is a *sample*, not the history, and nothing wider than the last 50 trades can be read from it. `Store.record_trades` mirrors each poll into a local `trades` table (primary key makes re-inserting a no-op), and every window — loss today/yesterday/week, the reward windows, and the strategy's daily count — is read back from that mirror via `Store.trades_between` / `count_trades`. The sweep runs far more often than a wallet trades 50 rows, so the mirror stays complete going forward; **days before the mirror started simply are not there.** Reading a window straight from a poll is the bug that made the week's loss collapse onto the day's once a wallet hit its 50-swap target, and that caps the strategy's daily target at 50.
...
- [wallets.py](src/cantex_bot/wallets.py) — one `CantexSDK` per wallet. Auth is **lazy** (`ensure_auth`), so hundreds of unused wallets cost nothing, and bounded by `_AUTH_TIMEOUT`: `sdk.authenticate()` holds an asyncio lock across three retrying requests, so an unbounded one parks every other caller for that wallet — the trading loop included. For the same reason `WebClient` caches the Bearer token instead of calling `authenticate` per request (which also made a probe request each time, doubling the read load). `_seed_session` deliberately reaches into `sdk._session` to install the tuned connector.
...
- [store.py](src/cantex_bot/store.py) — SQLite (`state.db`): swaps, daily counters, fee observations, the trading-history mirror, scrape snapshots. Coarse-locked, thread-safe. `pair_fee_stats()` returns `PairStats` — named fields, not a tuple, because eight numbers positionally was unreadable — with today's fee min/**max**/avg plus the latest slippage, pool fee and pool depth.
...
- [monitor.py](src/cantex_bot/monitor.py) — `FeeMonitor` quotes every pair on a loop and records fee/slippage/pool-fee/pool-depth. **It never swaps**: `get_swap_quote` is a pricing call, so watching the whole market costs nothing. Both directions are quoted because they are different numbers, with the sell side sized from the buy quote's own output so a pair is measured as a real round trip. Samples go to the same `fee_obs` table the trading side writes, so monitor and dashboard share one dataset. `both_ways=False` halves the sweep when only the buy side matters — at 20 pairs both ways a sweep takes ~39s and outlasts a 30s interval, so the real cycle is sweep+interval and there is no idle time; the dashboard says so when that happens. Its `MonitorDashboard` (in [dashboard.py](src/cantex_bot/dashboard.py)) paints from `FeeMonitor.pair_stats`, recomputed once per sweep off the event loop. Every sort puts the most attractive row on top, so direction differs per column (cheapest fee first, deepest pool first).
- [distribute.py](src/cantex_bot/distribute.py) — the mirror of `withdraw` (many→one): **one wallet to many receivers**. Destinations are either configured wallets — whose Canton party id is *not* in `config.toml`, so `internal_recipients` authenticates each one to read `AccountInfo.address` — or `recipients.txt` (gitignored; `recipients.example.txt` shows the forms, and a per-line amount overrides the shared one). Sent as **one `transfer` per recipient**. `batch_transfer` is the obvious fit and would cost a single network fee, but the API answers `401 {"error":"unauthorized address"}` for it — measured 2026-09-21, and it fails with one recipient while a plain `transfer` to that same address builds fine, so it is the endpoint that is closed, not the address. Two consequences callers must handle: **a network fee per recipient**, and a run that can end up **partially sent** — hence `SendResult` per address, `ok_total` (what succeeded) separate from `total` (the plan), and an `on_result` callback for live progress. The full planned total is still checked against the balance up front so a run that cannot finish never starts. A duplicate address in the file is rejected — it would silently double that payout. The CLI reads the sender's balances **before** asking for an amount (and only offers tokens it actually holds), then checks the total fits **before** arming: being told it does not fit after typing `LIVE` is three steps too late. `keep` stays in the module API but the CLI no longer asks for it — the amount here is absolute per recipient, so a reserve only confused the question (it is a percent-of-balance concept, from `withdraw`).
- [addresses.py](src/cantex_bot/addresses.py) — reads each wallet's Canton party id, which is not in `config.toml` but comes from `AccountInfo.address`, so every wallet authenticates once. `collect_addresses` returns `(found, failed)` and never raises for one bad wallet, because its two callers want opposite rules: the export names what it could not read (a short file that looks complete is the failure mode), while `distribute.internal_recipients` turns any failure into an error (a quietly shrunk payout is worse). Export writes one id per line with `# wallet-name` comments, which `parse_recipients` ignores — so an export is directly usable as a recipients list.
...
- [singleton.py](src/cantex_bot/singleton.py) — an advisory `flock` beside `state.db`, taken in `amain` before `App` exists. Two bots on the same wallets each read the balance, each decide to buy, and both submit; `_confirm_via_history` reconciles one process against the exchange and cannot see a second at all. The tell is trades at sizes the running bot never computed. Per-machine and per-directory only — it cannot catch a bot started from another checkout or host.
- [wallet_import.py](src/cantex_bot/wallet_import.py) — shared by `scripts/add_wallets.py` and the menu. `assert_gitignored` refuses to write secrets to a git-tracked file. `_misalignment` turns a broken dump into a location: the `Cantex::` address is the one unmistakable line in a block and belongs at offset 2 of every one, so the first marker off that grid names the wallet that is short or long a line. "got 194 (not a multiple of 5)" on its own left you counting lines by hand across forty wallets.

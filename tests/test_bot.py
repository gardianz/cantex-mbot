...
async def test_fetch_history_via_bearer():
    ...
    # One plain request: the endpoint ignores limit and offset entirely.
...
def _mtrade(dt, inp="USDCx", out="CBTC", ain="100", aout="1", cid="p"):
    ...
    return Trade(timestamp=dt, input_id=inp, input_admin="a", output_id=out,
                 output_admin="b", amount_input=Decimal(ain),
                 amount_output=Decimal(aout), pool_cid=cid)


def test_record_trades_is_idempotent(tmp_path):
    """Every sweep re-sees the same 50 rows; re-recording them must be a no-op
    or the mirror would double-count the loss."""
    from datetime import datetime, timezone
    store = Store(tmp_path / "s.db")
    day = datetime(2026, 8, 21, 4, tzinfo=timezone.utc)
    poll = [_mtrade(day, cid=f"p{i}") for i in range(3)]
    assert store.record_trades("w1", poll) == 3
    assert store.record_trades("w1", poll) == 0          # same poll again
    assert store.count_trades("w1", day.date(), day.date()) == 3
    store.close()


def test_mirror_accumulates_days_the_endpoint_drops(tmp_path):
    """The endpoint serves only the 50 newest rows, all from today. Polls taken
    on successive days must build up into a week the endpoint never returns."""
    from datetime import datetime, timedelta, timezone
    store = Store(tmp_path / "s.db")
    mon = datetime(2026, 8, 17, 12, tzinfo=timezone.utc)   # a Monday
    for d in range(5):                                     # Mon..Fri, one poll each
        store.record_trades("w1", [_mtrade(mon + timedelta(days=d), cid=f"c{d}")])
    fri = (mon + timedelta(days=4)).date()
    assert store.count_trades("w1", fri, fri) == 1         # today alone
    assert store.count_trades("w1", mon.date(), fri) == 5  # the whole week
    store.close()


def test_trades_between_is_per_wallet_and_ordered(tmp_path):
    from datetime import datetime, timedelta, timezone
    store = Store(tmp_path / "s.db")
    base = datetime(2026, 8, 21, 4, tzinfo=timezone.utc)
    store.record_trades("w1", [_mtrade(base + timedelta(minutes=5), cid="late"),
                               _mtrade(base, cid="early")])
    store.record_trades("w2", [_mtrade(base, cid="other")])
    got = store.trades_between("w1", base.date(), base.date())
    assert [t.pool_cid for t in got] == ["early", "late"]   # oldest first
    assert len(store.trades_between("w2", base.date(), base.date())) == 1
    store.close()


def test_mirror_round_trips_amounts_for_the_loss_helper(tmp_path):
    """Loss is computed off the mirror, so the amounts must survive SQLite."""
    from datetime import datetime, timedelta, timezone
    from cantex_bot.webclient import WebClient
    store = Store(tmp_path / "s.db")
    now = datetime(2026, 8, 21, 12, tzinfo=timezone.utc)
    store.record_trades("w1", _cycle(now, "CBTC", "100", "97"))
    day = now.date()
    loss = WebClient.loss_between(store.trades_between("w1", day, day),
                                  usdcx_symbol="USDCx", start=day, end=day)
    assert loss == Decimal("3")
    store.close()


@pytest.mark.asyncio
async def test_refresh_reads_windows_from_the_mirror_not_the_poll(monkeypatch):
    """Regression for LOSS d/w reading identical: the poll only ever holds
    today, so a week read straight from it collapses onto the day. Days already
    mirrored must still count."""
    from datetime import datetime, timedelta, timezone
    ...
    svc.store.record_trades("w1", _cycle(earlier, "CBTC", "100", "95"))
    wallet.web.fetch_trading_history = AsyncMock(
        return_value=_cycle(today, "CBTC", "100", "98"))
    monkeypatch.setattr(PortfolioService, "_to_cc",
                        staticmethod(lambda amt, price: amt))
...
@pytest.mark.asyncio
async def test_strategy_counts_today_from_the_mirror(tmp_path, monkeypatch):
    """The endpoint caps at 50 rows, so at a target of 50+ it can no longer see
    the whole day and the wallet would never reach its target. Counting from the
    mirror keeps the rows that fell off the poll."""
    from datetime import datetime, timezone
    strat, engine, rs, store = _strategy_fixture(tmp_path, monkeypatch, retries=3)
    today = datetime.now(timezone.utc).replace(hour=1)
    # 60 trades already mirrored; the poll can only ever hand back 50.
    store.record_trades("w1", [_mtrade(today, cid=f"c{i}") for i in range(60)])
    web = SimpleNamespace(fetch_trading_history=AsyncMock(
        return_value=[_mtrade(today, cid=f"c{i}") for i in range(50)]))
    wallet = SimpleNamespace(name="w1", web=web)
    del strat._web_swaps_today                    # use the real implementation
    assert await strat._web_swaps_today(wallet) == 60
    store.close()


def test_trades_older_than_the_retention_window_are_dropped(tmp_path):
    """The mirror never reads past this week, so it must not grow for ever."""
    from datetime import datetime, timedelta, timezone
    db = tmp_path / "s.db"
    store = Store(db)
    old = datetime.now(timezone.utc) - timedelta(days=90)
    recent = datetime.now(timezone.utc) - timedelta(days=1)
    store.record_trades("w1", [_mtrade(old, cid="old"), _mtrade(recent, cid="new")])
    store.close()
    store2 = Store(db)                            # pruning runs at open
    assert store2.count_trades("w1", old.date(), old.date()) == 0
    assert store2.count_trades("w1", recent.date(), recent.date()) == 1
    store2.close()
...
def _fake_portfolio():
    ...
    manager = SimpleNamespace(
        ...
        sem=_a.Semaphore(10))
    from cantex_bot.store import Store
    store = Store(":memory:")
    store.fee_stats_today = lambda n: (None, None, 0)      # type: ignore[method-assign]
    store.latest_fee = lambda n: Decimal("0.75")           # type: ignore[method-assign]
